...
class ManageSattaController extends Controller
{
     ...
     public function resultlog()
    {
        ...
        $resuldata = ResultData::where('game_type','1')->with('managesatta')->get();

        

        return view('admin.result-log',compact('resuldata'));
    }
...
//dd($inputdateclose);
    ...
    moneyDistrebute($manageSatta->first()->id,$request->flag,$request->pana);
      ...
      moneyDistrebutesattagames($manageSatta->first()->id,$request->flag,$request->pana,$request->open_input_result_date);
            }
        }
        	else if($request->flag == 'close')
        	{

 // $check =ResultData::where('satta_id',$request->id)->where('open_input_result_date',$request->close_input_result_date);

 //dd($check->first());

             if ($currentdate == $request->close_input_result_date ){
            ...
            moneyDistrebute($manageSatta->first()->id,$request->flag,$request->pana);
                ...
                else
                {
                   ...
                   if($manageSatta->first()->open_input_result_date == $request->close_input_result_date) {
                    
                $ResultData->update([
                    'open_pana' => $manageSatta->first()->open_pana,
                    'open_number' => $manageSatta->first()->open_number,
                    'open_aakda' => $manageSatta->first()->open_aakda,
                    'open_update' => $manageSatta->first()->open_update,
                    'game_type'  =>'1',
                    'open_input_result_date'=>$manageSatta->first()->open_input_result_date,
                ]);

                   }

                    if ($ResultData->first()->open_pana != null) {
                    ...
                    'close_input_result_date'=>$request->close_input_result_date,
  ...
  $manageSatta->update([
                ...
                moneyDistrebutesattagames($manageSatta->first()->id,$request->flag,$request->pana,$request->close_input_result_date);
