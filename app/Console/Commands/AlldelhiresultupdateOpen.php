...
class AlldelhiresultupdateOpen extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'AlldelhiresultupdateOpen:Open';
    ...
    $resultupdate = ResultData::where('open_update',$currentdatetime)->where('game_type','2')->get();
   ...
   foreach ($resultupdate as $key => $value) {
     ...
     \App\Model\ManageSatta::where('id',$value->satta_id)->where('status','y')->where('open_action','n')->update([
       ...
       moneyDistrebuteDelhiGame($value->satta_id,$value->open_pana);
