...
class AllstarresultupdateClose extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'AllstarresultupdateClose:Close';
   ...
   $resultupdate = ResultData::where('close_update',$currentdatetime)->where('game_type','3')->get();
   ...
   if ($resultupdate) {
     ...
     \App\Model\ManageSatta::where('id',$value->satta_id)->where('status','y')->where('close_action','n')->update([
       ...
       moneyDistrebutestarlinegame($value->satta_id,$type,$value->open_pana);
